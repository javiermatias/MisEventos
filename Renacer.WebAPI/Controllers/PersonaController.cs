...
namespace Renacer.WebAPI.Controllers
{
    ...
    {
        // GET: api/Personas
        public IEnumerable<Persona> Get([FromUri]int page, [FromUri] int limit, [FromUri] string search)
        {
            if (page <= 0) page = 1;
            if (limit <= 0) limit = 10;

            return ControlPersona.devolverInstancia().devolverTodos(page, limit, search);
        }
