...
export class ExcelService {
  ...
  public reporteExcel(_tituloWorkBook: string, _titulo: string, _nombreArchivo: String, _header: string[],
    _data: string[][], _filtros: string[]): Promise<string> {
    ...
    const promesa = new Promise<string>((resolver, rechazar) => {
      System.import('exceljs').then(Excel => {
        ...
        const worksheet = workbook.addWorksheet(_tituloWorkBook, { properties: { tabColor: { argb: 'FFC0000' } } });
        ...
        cellFiltros.font = { name: 'Comic Sans MS', family: 4, size: 12, bold: true }
        //Blank Row 
        worksheet.addRow([]);
        worksheet.addRow([]);
        //Add Header Row
