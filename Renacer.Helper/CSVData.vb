...
#Region " Load CSV "
    ...
    Public Sub LoadCSV(ByVal CSVFile As String, ByVal HasHeader As Boolean)
        ...
        Dim sr As New StreamReader(CSVFile, System.Text.Encoding.Default)
        Dim idx As Integer
    ...
    Private Sub ProcessLine(ByVal sLine As String)
        ...
        Do While sLine.Length > 0
            ...
            ElseIf regNormal.IsMatch(sLine) Then
                ...
                'sLine = sLine.Substring(sData.Length + 1)
...
#Region " Save As "
    ...
    ' Save data as CSV
    '
    Public Sub SaveAsCSV(ByVal sCSVFile As String, Optional ByVal IncluirHeaders As Boolean = False, Optional bAppend As Boolean = False)
        If dsCSV Is Nothing Then Exit Sub

        Dim dr As DataRow
        Dim sLine As String

        Dim sw As New StreamWriter(sCSVFile, bAppend, System.Text.Encoding.Default)
        Dim iCol As Integer

        If IncluirHeaders Then
            sLine = ""
            For i As Integer = 0 To dsCSV.Tables("CSVData").Columns.Count - 1
                If i > 0 Then sLine &= mSeparator
                If InStr(dsCSV.Tables("CSVData").Columns(i).ColumnName, mSeparator) > 0 Then
                    sLine &= mTextQualifier & dsCSV.Tables("CSVData").Columns(i).ColumnName & mTextQualifier
                Else
                    sLine &= dsCSV.Tables("CSVData").Columns(i).ColumnName
                End If
            Next
            sw.WriteLine(sLine)
        End If
        ...
        For Each dr In dsCSV.Tables("CSVData").Rows
            ...
            For iCol = 0 To dsCSV.Tables("CSVData").Columns.Count - 1
                ...
                If iCol > 0 Then sLine &= mSeparator
...
#Region " Properties "
    ...
    ' Dataset Property
    '
    Public Property CSVDataSet() As DataSet
        Get
            Return dsCSV
        End Get
